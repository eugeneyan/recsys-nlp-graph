...
if __name__ == '__main__':
    ...
    # Initialize model
    mf = MFBias(edges.n_unique_tokens, emb_dim)
    if torch.cuda.device_count() > 1:
        logger.info('Detected {} GPUs, using them all'.format(torch.cuda.device_count()))
        mf = torch.nn.DataParallel(mf)
    mf.to(device)
    ...
    for epoch in range(epochs):
        ...
        for i, batches in enumerate(dataloader):
            ...
            loss = mf.module.loss(pred, label)
            loss.backward()
            optimizer.step()
        ...
        state_dict_path = '{}/mf_bias_edges_epoch_{}_{}.pt'.format(MODEL_PATH, epoch, current_datetime)
    ...
    results_df.to_csv('{}/model_metrics_mf_bias_edges.csv'.format(MODEL_PATH), index=False)
