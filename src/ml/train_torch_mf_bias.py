...
if __name__ == '__main__':
    ...
    dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=shuffle, num_workers=args.n_workers,
                            collate_fn=dataset.collate_for_mf)
    ...
    # Initialize model
    mf = MFBias(sequences.n_unique_tokens, emb_dim).to(device)
    ...
    for epoch in range(epochs):
        ...
        state_dict_path = '{}/mf_bias_epoch_{}_{}.pt'.format(MODEL_PATH, epoch, current_datetime)
    ...
    results_df.to_csv('{}/model_metrics_mf_bias.csv'.format(MODEL_PATH), index=False)
